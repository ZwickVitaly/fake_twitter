...
from fake_twttr_app.db import Like, Tweet, User, async_session
...
async def delete_user(request: Request, admin_schema: AdminSchema):
    ...
    """
    logger.warning(
        f"Attempting user delete. User: {admin_schema.new_user_data.name} Admin: {admin_schema.login}"
    )
    async with async_session() as session:
        async with session.begin():
            ...
            if not deleted_user:
                logger.warning(
                    f"User {admin_schema.new_user_data.model_dump()} not found"
                )
                return JSONResponse(
                    status_code=404,
